...
import com.ivpulse.entity.*;
import com.ivpulse.entity.enums.*;
...
public final class ProductivvMapper {
    ...
    public static Client toClient(CostCenterDto dto) {
        ...
        ClientStatus status = (dto.getIsActive() != null && dto.getIsActive())
                ? ClientStatus.Active
        ...
        client.setClientName(dto.getClientName() != null
                ? dto.getClientName()
    ...
    public static Engagement toEngagement(ProjectDto dto, Client client) {
        ...
        EngagementStatus st = (end == null || !end.isBefore(LocalDate.now()))
    ...
    public static RoleMaster toRole(RoleRefDto dto) {
        ...
        role.setRoleId(id);
        role.setRoleName(dto.getTitle());
        role.setRoleCategory(null);
        role.setActiveFlag(dto.getIsActive() != null ? dto.getIsActive() : Boolean.TRUE);
    ...
    public static RateCard toRate(ProjectRateDto dto, Engagement engagement, RoleMaster role) {
        ...
        BigDecimal amount = dto.getOnshoreRate() != null
                ? dto.getOnshoreRate()
                : dto.getOffshoreRate();
    ...
    public static ResourceEntity toResource(EmployeeDto dto) {
        ...
        EmploymentStatus es = (dto.getIsActive() != null && dto.getIsActive())
    ...
    // --- EngagementResource ---
    public static EngagementResource toEngagementResource(
            ResourceAssignmentDto dto,
    ...
    // --- Billing snapshot ---
    public static BillingSnapshot toBillingSnapshot(
            BillingSnapshotDto dto,
            EngagementResource er) {
    ...
    // --- Onboarding status ---
    public static OnboardingStatus toOnboarding(OnboardingDto dto, ResourceEntity res) {

        BgvStatus bgv = null;
        if (dto.getBgvStatus() != null) {
            String s = dto.getBgvStatus().toLowerCase(Locale.ROOT);
            if (s.contains("progress"))
                bgv = BgvStatus.In_Progress;
            else if (s.contains("clear"))
                bgv = BgvStatus.Cleared;
            else
                bgv = BgvStatus.Not_Started;
        }

        OnboardingStatus os = new OnboardingStatus();
        os.setResource(res);
        os.setInductionStatus(null);
        os.setBgvStatus(bgv);
        os.setClientAccessStatus(null);
        os.setWorkOrderStatus(null);
        os.setComments(dto.getComments());
        os.setLastUpdatedAt(dto.getLastUpdatedAt());

        return os;
    }

    // --- Hiring status ---
    public static HiringStatus toHiring(
            HiringDto dto,
            ResourceEntity res,
            Engagement eng) {
        ...
        hsEntity.setExpectedJoiningDate(
                DateUtils.toLocalDate(dto.getExpectedJoiningDate()));
        hsEntity.setActualJoiningDate(
                DateUtils.toLocalDate(dto.getActualJoiningDate()));
    ...
    // --- Timesheet snapshot ---
    public static TimesheetSnapshot toTimesheetSnapshot(
            TimesheetDto dto,
            UUID resId,
            UUID engId) {
        ...
        boolean submitted = dto.getSubmittedAt() != null
                || (dto.getStatus() != null
                && dto.getStatus().toLowerCase(Locale.ROOT).contains("submit"));

        boolean approved = dto.getApprovedAt() != null
                || (dto.getStatus() != null
                && dto.getStatus().toLowerCase(Locale.ROOT).contains("approve"));
        ...
        ts.setApprovalDate(
                DateUtils.toLocalDate(dto.getApprovedAt()));
